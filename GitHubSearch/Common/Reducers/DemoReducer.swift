...
struct DemoReducer {
    ...
    func reduce(into state: inout State, action: AppReducer.Action) -> Effect<AppReducer.Action> {
        switch action {
        ...
        case let .repositories(.repositoriesResponse(.success(repositories))):
        ...
        case .repositories(.repositoryTapped):
            state.isDemoOngoing = false
        case .repositories(.repositoriesResponse(.failure)):
            state.isDemoOngoing = false
        default:
            return .none
        }
